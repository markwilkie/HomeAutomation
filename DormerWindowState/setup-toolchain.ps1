<#
.SYNOPSIS
  Installs ESP-IDF (native Windows) so the Dormer Window State Matter
  firmware can be built and flashed for the ESP32-C6.

.DESCRIPTION
  This project pulls in Matter as a managed component (espressif/esp_matter
  in main/idf_component.yml), the same approach the MiniSplit project uses,
  which builds fine on native Windows -- no WSL2 needed. This script:
    1. Enables Windows long-path support (registry; needs admin).
    2. Installs the Espressif Installation Manager (EIM) via winget.
    3. Uses EIM to install ESP-IDF v5.4.1, matching sdkconfig.defaults.
