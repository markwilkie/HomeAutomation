...
{
/*
	std::cout << "Controller: " << controller.c_str() << '\n';
	std::cout << "API: " << api.c_str() << '\n';
	std::cout << "Token: " << token.c_str() << '\n';
	std::cout << "Post Data: " << postData.c_str() << '\n';
	std::cout << "Type: " << type << '\n';
*/
	...
	if ( res != CURLE_OK)
	{
		LogLine() << "\nCurl ERROR: " << (int)res << " " << curl_easy_strerror(res) << '\n';
		LogLine() << "Long(er) Error: " << errbuf << '\n';
	...
	if(isHTTPError())
	{
		LogLine() << "Raw Response: " << webResponse.c_str() << '\n';
