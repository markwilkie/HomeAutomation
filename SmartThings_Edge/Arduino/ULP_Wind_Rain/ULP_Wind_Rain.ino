...
void blink(int blinkTotal,int delayTime)
{
...
void windTest() {
  ...
  if (timeSleep > 0) {
    ...
    // Wind Speed and Rain
    int rainPulsesRaw = getRainPulseCount();
    int windPulsesRaw = getWindPulseCount();
    int shortestWindPulse = getShortestWindPulse();
    ...
    //Calc wind
    float wind = (((float)windPulsesRaw / (float)timeSleep) * WINDFACTOR) / 20;  //there's 20 pulses per revolution 
...
static uint32_t getRainPulseCount(void) {
...
static uint32_t getWindPulseCount(void) {
  /* ULP program counts signal edges, convert that to the number of pulses */
...
static uint32_t getShortestWindPulse(void) {
