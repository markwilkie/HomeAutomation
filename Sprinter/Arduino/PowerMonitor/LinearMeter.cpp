...
void LinearMeter::drawMeter(char* label, int _vmin, int _vmax, int _scale, int _minorTicks, int _majorTicks, int _x, int _y, int _height, int _width)
{
    ...
    //Draw meter itself
    lcd.drawRect(x, y, _width, height, TFT_GREY);
    lcd.fillRect(x + METER_BORDER, y + METER_CAP_HEIGHT, width - (METER_BORDER*2), height - (METER_CAP_HEIGHT*2), TFT_WHITE);
    lcd.setTextColor(TFT_CYAN, TFT_BLACK);
    lcd.drawCentreString(label, x + _width / 2, y + 2, 2);
    ...
    for (int i = 0; i < minorTicks; i++) {
        lcd.drawFastHLine(x + (width*.55), y + tickStart+(tickSpacing*i), (width*.16), TFT_BLACK);
    }

    //Draw major ticks
    int ticksPerMajor=(minorTicks-1)/(_majorTicks-1);
    for (int i = 0; i <_majorTicks; i++) {
        lcd.drawFastHLine(x + (width*.55), y + tickStart+(ticksPerMajor*i*tickSpacing), (width*.25), TFT_BLACK);
    }    

    lcd.drawCentreString("-", x + _width / 2, y + height - 18, 2);
}
...
void LinearMeter::updatePointer(float value, int digits, int dec)
{
    ...
    //draw value
    lcd.fillRect(x + METER_BORDER, y + height - METER_CAP_HEIGHT, width - (METER_BORDER*2), METER_CAP_HEIGHT, TFT_BLACK);  //blank out old number
    lcd.setTextColor(TFT_GREEN, TFT_BLACK);
    char buf[8]; dtostrf(value, digits, dec, buf);
    lcd.drawCentreString(buf, x + width / 2, y + height - 18, 2);
    ...
    while(yValue != oldValue) 
    {
        ...
        if (oldValue > yValue) 
        {
            lcd.drawLine(dx, dy - (POINTER_HEIGHT/2), dx + POINTER_WIDTH, dy, TFT_WHITE);
            oldValue--;
            lcd.drawLine(dx, dy + (POINTER_HEIGHT/2) + 1, dx + POINTER_WIDTH, dy + 1, TFT_RED);
        }
        else 
        {
            lcd.drawLine(dx, dy + (POINTER_HEIGHT/2), dx + POINTER_WIDTH, dy, TFT_WHITE);
