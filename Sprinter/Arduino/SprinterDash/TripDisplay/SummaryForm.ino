...
void SummaryForm::updateDisplay()
{
  ...
  strField.updateField(geniePtr,DRIVING_TIME_STRING, drivingTime, tripSegDataPtr->getDrivingTime(),sizeof(drivingTime)-1);
  strField.updateField(geniePtr,ELASPED_TIME_STRING, elapsedTime, tripSegDataPtr->getElapsedTime(),sizeof(elapsedTime)-1);
  strField.updateField(geniePtr,TIME_STOPPED_STRING, stoppedTime,tripSegDataPtr->getStoppedTime(),sizeof(stoppedTime)-1);
  strField.updateField(geniePtr,NUM_STOPS_STRING, numberOfStops, tripSegDataPtr->getNumberOfStops(),2);
  strField.updateField(geniePtr,TIME_PARKED_STRING, parkedTime, tripSegDataPtr->getParkedTime(),sizeof(parkedTime)-1);
  ...
  strField.updateField(geniePtr,ELEVATION_GAIN_STRING, elevationGain, tripSegDataPtr->getTotalClimb(),sizeof(elevationGain)-1);
}
